...
%Diagnostics dialog
 mainmenu:-
    new(D,dialog('Covid-19 Predict')),send(D,append,new(label)),
    ...
    send(D,append,new(Headache,menu('Have headache?',marked))),
    send(D,append,new(Drycough,menu('Have dry cough?',marked))),
    send(D,append,new(Fatigueness,menu('Easily fatigued?',marked))),
    send(D,append,new(Aches ,menu('Have aches?',marked))),
    send(D,append,new(Losstest ,menu('Loss test or smell?',marked))),
    ...
    send(Aches, append, yes),   send(Aches, append, no),
    send(Losstest, append, yes),   send(Losstest, append, no),
    ...
    Headache?selection,Aches?selection,Losstest?selection,
...
save_main(Name,Fever,Asthma,Headache,Aches,Losstest,Fatigueness,Drycough,Diabetes,Sex,Location,Celsius):-

        new(A,dialog('Covid-19 Predict Results')),
        ...
        (Headache == 'yes' -> Headachevalue is 1;Headachevalue is 0),

        (Aches == 'yes' -> Achesvalue is 1;Achesvalue is 0),

        (Losstest == 'yes' -> Losstestvalue is 1;Losstestvalue is 0),
        ...
        %Add risk values
        Covid_19 is Fevervalue+Asthmavalue+Headachevalue+Achesvalue+Losstestvalue+Fatiguevalue+Drycoughvalue+Diabetesvalue+Tempval+Locationvalue,

        %If total risk value >= 4 patient at risk
        send(A,append,new(Lbl15,label)),
        (Covid_19  >= 4 -> send(Lbl15,append,'!!! You are at risk of Covid-19'),
